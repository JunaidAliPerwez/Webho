...
class HomeController extends Controller
{
    ...
    {
        $user = Auth::user();
        $inquiry = inquiry::orderBy('id', 'DESC')->simplePaginate(10);
        $inquiryProject = inquiryProject::orderBy('id', 'DESC')->simplePaginate(10);

        return view('web.dashboard')->with(compact('user' ,'inquiry', 'inquiryProject'));

    }

    public function inquiry_create_update_delete(Request $request){
        ...
        if($dataCreate == 1){
            ...
            $inquiry->name = $name;
            $inquiry->email = $email;
            $inquiry->phonenumber = $phonenumber;
            $inquiry->message = $message;
            $inquiry->url = $url;

            $inquiry->save();

            return redirect()->route('dashboard')->with('success', 'Inquiry create successfully');
        ...
        if($dataUpdate == 1){
            ...
            $inquiry = inquiry::where('id', $id)->update($data);

            return redirect()->route('dashboard')->with('success', 'Inquiry updated successfully');
        }
    ...
    public function inquiry_project_create_update_delete(Request $request){
        $data = array();
        ...
        if($dataCreate == 1){
            ...
            return redirect()->route('dashboard')->with('success', 'Project inquiry create successfully');
        ...
        if($dataUpdate == 1){
            ...
            $inquiry = inquiryProject::where('id', $id)->update($data);

            return redirect()->route('dashboard')->with('success', 'Project inquiry updated successfully');
        }

        // delete
        if($dataDelete == 1){

            $inquiry = inquiryProject::where('id', $id)->delete();

            return redirect()->route('dashboard')->with('success', 'Project inquiry deleted successfully');
        }
    }
